...
function circStatsSummary = plotPhaseDiff(phaseData, params, jPCplane)
    ...
    if ~exist('params', 'var') || ~isfield(params,'suppressHistograms') || ~params.suppressHistograms
        figure;
        current_pdf = histogram([phaseData.phaseDiff], bins, 'Normalization', 'pdf', 'FaceColor', 'b', 'FaceAlpha', 0.5, 'EdgeAlpha', 0.5); hold on;
        plot(circMn, 1, 'ro', 'markerFa', 'r', 'markerSiz', 8);
        plot(pi/2*[-1 1], [0 0], 'ko', 'markerFa', 'r', 'markerSiz', 8);
        set(gca,'XLim',pi*[-1 1]);
        ax = gca; x_coord = linspace(ax.XLim(1), ax.XLim(2), 1000); plot(bins, normpdf([phaseData.phaseDiff]));
