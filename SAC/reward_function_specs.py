...
def reward_function(state_td, data, model, action_t, threshold):
        ...
        for musculo_body_tracking in musculo_tracking:
            musculo_body = data.xpos[model.body(musculo_body_tracking[0]).id].copy()
            musculo_target = data.xpos[model.body(musculo_body_tracking[1]).id].copy()
            #print(musculo_body, musculo_target)
